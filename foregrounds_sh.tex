...
A minimum compelling target for the CMB Probe is to constrain the tensor to scalar ratio with $\sigma(r) \leq 0.001$. Data from \planck\ and 
sub-orbital experiments reveal that incomplete knowledge of the foregrounds, not instrument noise, currently limits the measurements. 
Reducing $r$ uncertainties from their $\sim0.1$ level will have to be accompanied by equivalent higher fidelity measurements of the foregrounds. 
...
A minimum compelling target for the CMB Probe is to constrain the tensor to scalar ratio with $\sigma(r) \leq 0.001$. Data from \planck\ reveals that 


By the time the CMB Probe launches, substantial progress will already have been made in measurements of the polarization from sub
In the case of a detection at the time of launch, the probe mission should be able to convincingly detect the expected reionization signal. In the absence of a signal it should significantly improve the upper limits beyond $\sigma(r)=0.001$. 
