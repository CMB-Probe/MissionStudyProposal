...
\subsection{Why Space? } 

To reach the science goals outlined in Section~\ref{sec:science} the Probe, 
or any equivalent instrument, will need full sky coverage to reach 
the lowest $\ells$, sensitivity 
to reach at least $\sigma(r) < 0.001$, capability to remove foregrounds 
that are expected to be a factor of 100 larger than target $B$ mode levels, 
and control of systematic uncertainties to a level that is at least a factor 10 lower
that the target $r$ level. 
Three previous CMB satellites and many sub-orbital experiments have repeatedly demonstrated that a space-based platform is by far the best platform for realizing these requirements. 

Significant progress in measurements of the CMB polarization is made by sub-orbital measurements. 
...
The CMB Probe has 
complete flexibility in the choice of frequency coverage, both of which are challenging from the ground
